...
coxVariableName <- function(object, model.frame){
  ...
  if(n.xterms>n.specials){
    ...
    if (any(class(object)%in%c("glmnet"))){
      out$lpvars <- object$beta@Dimnames[[1]]
    }else{
...
## ** coxCenter.glmnet
#' @rdname coxCenter
#' @method coxCenter glmnet
#' @export
coxCenter.glmnet <- function(object){
  return(apply(object$call$x,2,mean))
}
# }}}
...
coxModelFrame.glmnet <- function(object, center = FALSE){
  ...
  if(center){
    dt <- as.data.table(rowCenter_cpp(object$call[["x"]], center = coxCenter(object)))
  }else{
    dt <- as.data.table(object$call[["x"]])
  }
  ...
  ## ** add y
  if(is.null(dt)){
    dt <- data.table(status = object$call[["y"]][,"status"])
  }else{
    dt[,c("status") := object$call[["y"]][,"status"]]
  }
  if("start" %in% colnames(object$y) == FALSE){
    dt[,c("start","stop") := list(default.start, object$call[["y"]][,"time"])]
  }else{
    dt[,c("start","stop") := list(object$call[["y"]][,"start"], object$call[["y"]][,"stop"])]
  }

  ## ** add strata
  if("strata" %in% names(object)){
    dt[,c("strata") := object$call[["strata"]]]
  }else{
    dt[,c("strata") := factor(1)]
  }
...
#' @method coxLP glmnet
#' @export
coxLP.glmnet <- function(object, data, center){
  ...
  if(n.varLP>0){
    ...
    if(is.null(data)){ ## training dataset
      X <- object$call$x
      Xb <- as.vector(X[,rownames(coef), drop = FALSE] %*% coef)
    }else{
      X <- model.matrix(object$call$formula[-2], data)[,-1] # drop intercept column
      Xb <- as.vector(X[,rownames(coef), drop = FALSE] %*% coef)
    }
